...
const HeroCarousel = () => {
  ...
  useEffect(() => {
    ...
    slides.forEach((slide) => {
      const img = new Image();
      const imageUrl =
        windowWidth < 768 && slide.mobileImage
          ? slide.mobileImage
          : slide.image;
      img.src = imageUrl;
      img.onload = () => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-[#0a0a0a]">
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 py-4 sm:py-6 md:py-8 lg:py-10 px-4 sm:px-6 md:px-10 lg:px-16 bg-black/80">
        <div className="max-w-7xl mx-auto">
          <AnimatePresence mode="popLayout" custom={direction}>
            ...
            >
              {/* Slogan with trendy font */}
              <p
                className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-light tracking-wide text-white"
                style={{
              ...
              >
                <svg
                  className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 text-amber-400/80"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
      ...
      >
        <svg
          className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white/60 group-hover:text-white transition-colors"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
